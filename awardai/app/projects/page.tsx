...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    ...
    // Fetch org_id first, then query projects by org_id.
    // This handles both new projects (created_by set) and older ones where
    // user_id was used instead of created_by — org_id is always set on every project.
    supabase
      .from('profiles')
      .select('org_id')
      .eq('id', user.id)
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <header className="border-b border-gray-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-white">AwardAI</span>
        </div>
        <button
          ...
          <div className="grid gap-3">
            {projects.map((p) => (
              <button
                ...
                className="w-full text-left bg-gray-900 hover:bg-gray-800 border border-gray-800 rounded-xl p-5 transition-colors"
              >
                <div className="flex items-start justify-between">
                  <div>
                    <h2 className="font-medium text-white">{p.campaign_name}</h2>
                    {p.client_name && (
                    ...
                    {p.target_shows?.length > 0 && (
                      <div className="flex gap-2 mt-2 flex-wrap">
                        ...
                        {p.target_shows.length > 3 && (
                          <span className="text-xs text-gray-500">+{p.target_shows.length - 3} more</span>
                        )}
                      </div>
                    )}
                  </div>
